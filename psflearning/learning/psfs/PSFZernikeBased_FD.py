...
class PSFZernikeBased_FD(PSFZernikeBase):
    """
    ...
    def calc_forward_images(self, variables: list) -> tf.Tensor:
        ...
        for i in range(0, Zmap.shape[-3]):
            Zcoeff1[i] = tfp.math.batch_interp_regular_nd_grid(
                cor[self.ind[0]:self.ind[1], -2:], [0.0, 0.0], np.float32(imsz[-2:]), Zmap[0, i], axis=-2
            )
            Zcoeff2[i] = tfp.math.batch_interp_regular_nd_grid(
                cor[self.ind[0]:self.ind[1], -2:], [0.0, 0.0], np.float32(imsz[-2:]), Zmap[1, i], axis=-2
            )
