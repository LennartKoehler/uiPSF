...
class PSFZernikeBased(PSFZernikeBase):
    ...
    def __init__(self, options: Any = None) -> None:
        ...
        self.psf_type = 'scalar'
        self.optimization_weights: OptimizationWeights = None
    ...
    def calc_initials(self, data: PreprocessedImageDataInterface, start_time: float = 0.0) -> tuple[ZernikePSFVariables, Any]:
        ...
        self.gen_bead_kernel(data)
        n_beads = rois.shape[0]
        n_z_slices = rois.shape[-3]

        self.calpupilfield(data, self.psf_type)

        if self.params.model.const_pupilmag:
            self.max_magnitude_order = 0
        else:
            self.max_magnitude_order = 100
        ...
        self.optimization_weights = OptimizationWeights(
        ...
        sigma = np.ones((2,)) * self.params.model.blur_sigma * np.pi
        ...
        init_zernike_coeff_magnitude[0, 0, 0] = 1 / self.optimization_weights.zernike_magnitude

        init_backgrounds = (
            np.ones((n_beads, 1, 1, 1), dtype=np.float32)
            * np.median(init_backgrounds, axis=0, keepdims=True)
            / self.optimization_weights.background
        )
        ...
        if self.params.model.var_photon:
            init_intensity = init_intensity_grid / self.optimization_weights.intensity
        else:
            init_intensity = init_intensities / self.optimization_weights.intensity
    ...
    def calc_forward_images(self, variables, data=None) -> tf.Tensor:
        ...
        if self.initial_pupil is not None:
            ...
            pupil = self.compute_pupil_from_zernike(
                ...
                self.optimization_weights.zernike_magnitude, self.optimization_weights.zernike_phase,
            )
        ...
        phase_z, phase_xy = self._compute_phase(
            ...
            self.frequency_y,
            self.frequency_z,
            self.frequency_z_medium,
        )

        propagated_psf_intensity = self._render_psf(
            pupil, phase_z, sigma, phase_xy=phase_xy,
            use_bead_kernel=True, data=data,
        )

        if self.params.model.estimate_drift:
            drift_xy = drift_xy * self.optimization_weights.drift
            forward_images = self.applyDrift(propagated_psf_intensity, drift_xy, data) * intensities * self.optimization_weights.intensity + backgrounds * self.optimization_weights.background
        else:
            forward_images = propagated_psf_intensity * intensities * self.optimization_weights.intensity + backgrounds * self.optimization_weights.background
    ...
    def genpsfmodel(
        ...
        phiz = -1j * 2 * np.pi * self.frequency_z * (self.z_range + self.defocus)
        phase_z = tf.exp(phiz)

        psf_model_image = self._render_psf(pupil, phase_z, sigma, use_bead_kernel=addbead)

        return psf_model_image, pupil

    def postprocess(self, data: PreprocessedImageDataSingleChannel, variables: ZernikePSFVariables) -> ZernikePSFResult:
        """
        ...
        z_center = (self.z_range.shape[-3] - 1) // 2

        zernike_coeff_magnitude = zernike_coeff_magnitude * self.optimization_weights.zernike_magnitude
        zernike_coeff_phase = zernike_coeff_phase * self.optimization_weights.zernike_phase

        bin_factor = self.params.model.bin
        ...
        return ZernikePSFResult(
            ...
            backgrounds=backgrounds * self.optimization_weights.background,
            intensities=intensities * self.optimization_weights.intensity,
