...
class PSFZernikeBased_FD_smlm(PSFZernikeBase):
    """
    ...
    def calc_forward_images(self, variables: list) -> tf.Tensor:
        ...
        for i in range(0, Zmap.shape[-3]):
            Zcoeff1[i] = tfp.math.batch_interp_regular_nd_grid(
                cor[self.ind[0]:self.ind[1], -2:], [0, 0], imsz[-2:], Zmap[0, i], axis=-2
            )
            Zcoeff2[i] = tfp.math.batch_interp_regular_nd_grid(
                cor[self.ind[0]:self.ind[1], -2:], [0, 0], imsz[-2:], Zmap[1, i], axis=-2
            )
        ...
        if self.options.model.symmetric_mag:
            ...
            if self.options.model.zernike_nl:
                pupil_mag = tf.reduce_sum(
                    self.Zk[self.noll_index] * tf.gather(Zcoeff1, indices=self.noll_index, axis=1), axis=1
                )
            else:
        ...
        pupil = tf.complex(
            pupil_mag * tf.math.cos(pupil_phase),
            pupil_mag * tf.math.sin(pupil_phase),
        ...
        phiz = 1j * 2 * np.pi * (
            self.kz_med * pos[:, 0] * self.zweight[self.ind[0]:self.ind[1]] - self.kz * stagepos
        )
    ...
    def genpsfmodel(self, sigma: np.ndarray, Zcoeff: np.ndarray | None = None, Zmap: np.ndarray | None = None, cor: np.ndarray | None = None, stagepos: np.ndarray | None = None, pupil: Any = None) -> tuple[np.ndarray, Any, Any]:
        ...
        if Zmap is not None:
            ...
            for i in range(0, Zmap.shape[-3]):
                Zcoeff1[i] = tfp.math.batch_interp_regular_nd_grid(
                    cor, [0.0, 0.0], np.float32(imsz[-2:]), Zmap[0, i], axis=-2
                )
                Zcoeff2[i] = tfp.math.batch_interp_regular_nd_grid(
                    cor, [0.0, 0.0], np.float32(imsz[-2:]), Zmap[1, i], axis=-2
                )
